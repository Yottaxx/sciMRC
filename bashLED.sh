...
for item in "all withoutExtractive withoutGenerative withoutUnanswerable withoutYes withoutTask1 withoutTask2 withoutTask3"
do
  ...
  --validation_file ./filter_data/annotate/cleanDev$item.json \
  --test_file ./filter_data/annotate/cleanTest$item.json \
  ...
  --train_file ./filter_data/annotate/cleanTrain$item.json \
  --validation_file ./filter_data/annotate/cleanDev$item.json \
  --test_file ./filter_data/annotate/cleanDev$item.json \
  --max_source_length 4096 \
  --max_target_length 128 \
  --pad_to_max_length false \
  --source_prefix "Question Answering: " \
